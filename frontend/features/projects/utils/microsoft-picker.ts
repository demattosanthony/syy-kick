...
export class MicrosoftPicker {
    ...
    private async getSharePointConfig(options: PickerOptions, userToken: Token): Promise<object> {
        if (options.mode === "folder") {
            const orgDrive = await microsoftApi.graph.getOrgDrive(userToken.accessToken);
            if (orgDrive.webUrl) {
                return {
                    byPath: {
    ...
    private async displayPicker(pickerOptions: object, userToken: Token): Promise<void> {
    ...
    public async pickerSelectionsToFiles(pickerFiles: SharePointFile[]): Promise<File[]> {
    ...
    private async downloadFile(pickerObj: SharePointFile): Promise<File | null> {
        ...
        const data = await microsoftApi.graph.getFile(parentReference.driveId, id, this.token.accessToken);
