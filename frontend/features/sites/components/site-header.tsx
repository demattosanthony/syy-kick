...
export default function SiteHeader({ site }: { site: Site }) {
  ...
  const mapUrl = useMemo(() => {
    ...
    return `https://maps.googleapis.com/maps/api/staticmap?center=${site.latitude},${site.longitude}&zoom=17&size=400x200&maptype=satellite&key=${process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY}`;
  }, [site]);
      ...
      <div className="absolute inset-0 flex items-start justify-between">
        <div className="relative flex flex-1 items-center justify-start p-4 gap-2">
          ...
          <p className="text-xl font-bold text-muted-foreground text-white">
            {site.address} {site.city}, {site.state} {site.postalCode}, {site.country}
          </p>
        </div>
      </div>
    </div >
