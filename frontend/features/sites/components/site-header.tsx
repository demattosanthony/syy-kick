...
export default function SiteHeader({ site }: { site: Site }) {
    ...
    return (
        <div className="relative h-[160px] w-full bg-muted rounded-lg overflow-hidden">
            <img src={mapUrl} alt={site.name} className="object-cover w-full h-full" loading="lazy" />
            {/* Gradient overlay */}
            ...
            <div className="absolute inset-0 flex items-start justify-between">
                <div className="relative flex flex-1 flex-col items-start justify-start p-4">
                    <p className="text-xl font-bold text-muted-foreground text-white">{site.name}</p>
                    <div className="flex flex-col">
                        <p className="text-sm font-medium text-muted-foreground text-white">{site.address.address}</p>
                        <p className="text-sm font-medium text-muted-foreground text-white">{site.address.city}, {site.address.state} {site.address.postalCode}</p>
                    </div>
                </div>
                <div className="flex flex-col items-end justify-end p-4">
                    <DropdownMenu modal={false}>
                        ...
                        <DropdownMenuContent>
                            <SiteMutationDialog
                                trigger={
                                    <DropdownMenuItem disabled={!canUpdateOrgSites} onSelect={(e) => e.preventDefault()} className="hover:cursor-pointer">
                            ...
                            />
                            <DropdownMenuItem disabled={!canDeleteOrgSites} onSelect={() => setShowDeleteDialog(true)} className="text-destructive hover:cursor-pointer">
