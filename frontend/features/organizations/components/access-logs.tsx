                ...
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <CardDescription>Monitor and analyze system access patterns</CardDescription>
                        </div>
                                        ...
                                        {filters.resource !== "all" && (
                                            <div className="flex items-center gap-2 min-w-0">
                                                <span className="flex-shrink-0">
                                                    <ResourceIcon resource={filters.resource as Permissions.Resources} />
                                                </span>
                            ...
                            <Select value={filters.action} onValueChange={(value) => handleFilterChange("action", value)}>
                                ...
                                <SelectContent>
                                    ...
                                    {actions.map(([key, value]) => (
                                        <SelectItem key={key} value={value}>
                                            <Badge variant="outline" className={getActionColor(value)}>{actionsTranslations[value]}</Badge>
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                            ...
                            <Select value={filters.status} onValueChange={(value) => handleFilterChange("status", value)}>
                                ...
                                <SelectContent>
                                    ...
                                    {status.map(([key, value]) => (
                                        <SelectItem key={key} value={value}>
                                            <Badge variant="outline" className={getStatusColor(value)}>{accessLogStatusTranslations[value]}</Badge>
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                            ...
                            <TableBody className="transition-all duration-300 min-h-[400px]">
                                {(isLoading || isFiltering) ? (
                                    ...
                                    filteredLogs.map((log) => (
                                        <TableRow key={log.id} className="transition-all duration-300">
                                            <TableCell className="whitespace-nowrap">{format(new Date(log.createdAt), "MMM d, yyyy HH:mm")}</TableCell>
                                            <TableCell>
                                                <div className="flex items-center gap-2">
                                                    <Avatar className="h-8 w-8 rounded-full">
                                                        <AvatarImage src={log.user?.profilePicture} alt={log.user.name} />
                                            ...
                                            <TableCell>
                                                <div className="flex items-center gap-1.5">
                                                    <Badge variant="outline" className="bg-primary/10 rounded-full p-1">
                                                        <ResourceIcon resource={log.resource.name as Permissions.Resources} />
                                                    </Badge>
                                                    <span>{resourceNameToLabel(log.resource.name)}</span>
                                                </div>
                                            </TableCell>
                                            <TableCell>
                                                <Badge variant="outline" className={getActionColor(log.action.name)}>
