...
import {
  type WorkflowRunStepOutput,
  type WorkflowFile,
  type StepOutputValue,
  type StepContext,
  type TreeNode,
  StepStatus,
  CustomWorkflowRun,
...
export function getFileIcon(mimeType: string, url: string) {
  switch (true) {
    case mimeType.startsWith("image/"):
      return (
        <img
          ...
          width={105}
          alt="File Icon"
          className="p-1 h-[105px] w-[105px] object-cover min-h-[105px] min-w-[105px] rounded-md"
        />
      );
    ...
    case ["application/pdf"].includes(mimeType):
      return <img src={pdf} alt="PDF" className="h-18 w-18 object-cover" />;
...
export function renderFile(file: WorkflowFile) {
  return (
    <div className="flex flex-col gap-1 w-28" key={file.fileKey}>
      <a
        href={file.url}
        target="_blank"
      ...
      <p className="text-xs text-center w-full line-clamp-2">{file.fileName}</p>
    </div>
  );
}
...
function renderOutputArray(array: any[], label: string) {
  ...
  const containsSchemaTypes = array.some(
    (item) =>
      ...
      typeof item === "object" &&
      "type" in item &&
      ["text", "file", "number"].includes(item.type)
  );
  ...
  if (containsSchemaTypes) {
    return (
      <div className="space-y-2">
        <div className="flex flex-wrap gap-4">
          {array.map((item) => renderSingleOutput(item))}
        </div>
      </div>
    );
  }
...
export function buildTree(
  ...
  graph.forEach((entry) => {
    switch (entry.type) {
      ...
      case "parallel":
      case "conditional":
        nodes.push(...buildTree(entry.steps, ctx, parentPath));
        break;
      ...
      case "foreach":
        pushStep(entry.step, "foreach", {
          foreachConcurrency: entry.opts.concurrency,
        });
        break;
    }
  });
