...
export function WorkflowRunInput({
  ...
  const renderInputValue = (
    value: VNextWorkflowRunState["context"]["input"]
  ) => {
  ...
  return (
    ...
    >
      <AccordionItem value="inputs" className="border-none">
        <AccordionTrigger className="px-4 py-4 hover:bg-muted cursor-pointer">
        ...
        <AccordionContent className="px-4 pb-4 border-t border-border">
          <div className="space-y-4">
            {runState.snapshot.context.input &&
              Object.entries(runState.snapshot.context.input).map(
                ([key, value]) => (
