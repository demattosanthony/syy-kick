...
import { FileText, Type, Hash, Calendar, List, Check, AlertCircle } from "lucide-react";
import {
    Command,
    CommandEmpty,
    CommandGroup,
...
const fieldTypes = [
    { type: 'text', label: 'Text Field', icon: Type, description: 'A simple text input field' },
    { type: 'number', label: 'Number Field', icon: Hash, description: 'A field for numeric values' },
    { type: 'date', label: 'Date Field', icon: Calendar, description: 'A field for date selection' },
    { type: 'file', label: 'File Upload', icon: FileText, description: 'A field for file uploads' },
    { type: 'select', label: 'Select Field', icon: List, description: 'A dropdown selection field' },
];
...
const WorkflowStep = ({
    ...
    const getFieldError = (fieldPath: string[]) => {
        return errors[0]?.issues.find(err =>
    ...
    const hasErrors = errors[0]?.issues.some(err =>
        err.path[0] === "workflowSteps" &&
        err.path[1] === index
    ) || false;
        ...
        <div className="space-y-6">
            <Tabs defaultValue={step.agentId ? "agent" : "custom"} className="w-full">
                <TabsList className="grid w-full grid-cols-2">
                    <TabsTrigger value="agent" onClick={() => onUpdateStepField(step.id, "agentId", null)}>
                        Use Existing Agent
                    </TabsTrigger>
                    <TabsTrigger value="custom" onClick={() => onUpdateStepField(step.id, "agentId", null)}>
                        Custom Agent
                    </TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="agent" className="space-y-4">
                    <div className="mb-4">
                        ...
                        >
                            ...
                            <SelectContent>
                                {Object.entries(agentsTypesLabels).map(([type, { label, icon: Icon }]) => {
                    ...
                    {step.formSchema && (
                        <div className="mt-4">
                            <div className="mb-4">
                                <h3 className="text-lg font-semibold mb-2">Agent Input Fields</h3>
                                <p className="text-sm text-muted-foreground">Configure the input fields that will be shown to users during workflow execution.</p>
                            </div>
                            <div className="space-y-4">
                                {Object.entries(step.formSchema.fields).map(([fieldKey, field]) =>
                                    <FormField
                                        key={fieldKey}
                                        fieldKey={fieldKey}
                                        field={field}
                ...
                <TabsContent value="custom" className="space-y-4">
                    {hasErrors && (
                        <div className="p-4 bg-destructive/10 border border-destructive rounded-lg mb-4">
                            ...
                            <ul className="text-sm text-destructive/80 space-y-1">
                                {errors[0]?.issues
                                    .filter(err => err.path[0] === "workflowSteps" && err.path[1] === index)
                                    .map((err, i) => (
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            />
                            {getFieldError(["name"]) && (
                                <p className="text-sm text-destructive mt-1">{getFieldError(["name"])?.message}</p>
                            )}
                        </div>
                        ...
                        <div>
                            ...
                            {getFieldError(["description"]) && (
                                <p className="text-sm text-destructive mt-1">{getFieldError(["description"])?.message}</p>
                            )}
                        </div>
                        ...
                        <div>
                            ...
                            <Textarea
                                ...
                                className={cn(getFieldError(["instructions"]) && "border-destructive")}
                            />
                            {getFieldError(["instructions"]) && (
                                <p className="text-sm text-destructive mt-1">{getFieldError(["instructions"])?.message}</p>
                            )}
                        </div>
                        ...
                        <ModelSelector
                            step={step}
                            models={models || []}
                            onModelChange={(modelName) => handleFieldChange("model", modelName)}
                            hasError={!!getFieldError(["model"])}
                        />
                        {getFieldError(["model"]) && (
                            <p className="text-sm text-destructive mt-1">{getFieldError(["model"])?.message}</p>
                        )}
                                ...
                                <PopoverTrigger asChild>
                                    ...
                                    >
                                        {step.activeTools.length > 0
                                            ? `${step.activeTools.length} tool${step.activeTools.length > 1 ? 's' : ''} selected`
                                            : "Select tools"}
                                    </Button>
                                </PopoverTrigger>
                                                ...
                                                <CommandItem
                                                    ...
                                                    onSelect={() => {
                                                        ...
                                                        if (currentTools.includes(tool.id)) {
                                                            onUpdateStepField(step.id, "activeTools", currentTools.filter(t => t !== tool.id));
                                                        } else {
                                                            onUpdateStepField(step.id, "activeTools", [...currentTools, tool.id]);
                                                        }
                                                    }}
                                                >
                                        ...
                                        return tool ? (
                                            <div key={toolId} className="flex items-center gap-1 bg-secondary px-2 py-1 rounded-md text-sm">
                                                ...
                                                <button
                                                    onClick={() => {
                                                        const currentTools = step.activeTools;
                                                        onUpdateStepField(step.id, "activeTools", currentTools.filter(t => t !== toolId));
                                                    }}
                        ...
                        <div className="space-y-4">
                            ...
                            <Dialog open={isFieldTypeDialogOpen} onOpenChange={setIsFieldTypeDialogOpen}>
                            ...
                            {step.formSchema?.fields && Object.entries(step.formSchema.fields).map(([fieldKey, field]) => (
                                <FormField
                                    key={fieldKey}
                                    fieldKey={fieldKey}
                                    field={field}
                                    ...
                                    onDeleteField={!step.agentId ? handleDeleteField : undefined}
                                />
                            ))}
                        </div>
                        {step.formSchema && Object.keys(step.formSchema?.fields || {}).length === 0 && (
