    ...
    */
    ...
    getDocuments: async (req: Request, res: Response) => {
        try {
            ...
            const files = await documentsOps.getProjectDocs(projectId, path as string);
            res.json(files);
        } catch (error) {
    ...
    deleteContents: async (req: Request, res: Response) => {
        ...
        const { path } = req.query;
        await documentsOps.deleteProjectContent(projectId, decodeURIComponent(path as string));
        res.json({ success: true });
    },
}