import { and, asc, cosineDistance, eq, inArray, isNull, like, or, sql } from "drizzle-orm";
import db from "../../../config/db";
import { projectsOps } from "../projects.ops";
import { normalizePath } from "./documents.utils";
import { documentEmbeddings, documents, projects } from "../../../config/schema";
...
export const documentsOps = {
    ...
    createFolderStructure: async (projectId: string, data: z.infer<typeof documentsSchemas.docsUpload>) => {
        ...
        for (const entry of data.entries) {
            ...
            let parentId: string | null = null;
            if (fullPath.includes("/")) {
                parentId = await documentsOps.ensureParentFolderExists(projectId, fullPath);
            }
