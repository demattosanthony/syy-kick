      ...
      try {
        ...
        if (groundingMetadata?.groundingSupports?.length) {
          ...
          const supports = [...groundingMetadata.groundingSupports].sort(
            (a, b) =>
              (b.segment?.startIndex ?? 0) - (a.segment?.startIndex ?? 0)
          );
        ...
        console.error(`[WebSearchTool] Error processing ${query}:`, error);
        return {
          success: false,
          message: `Failed to search the web for '${query}': ${error instanceof Error ? error.message : "Unknown error"}`,
        };
      }
    },
  });
