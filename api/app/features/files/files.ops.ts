...
export const filesOps = {
  ...
  async processAndStoreFile(params: {
    userId: string;
    fileName: string;
    mimeType: string;
    size: number;
    ...
    try {
      ...
      if (category) {
        ...
        console.log(
          `📂 [ProcessAndStoreFile] File categorized as: ${category}`
        );
      }
  ...
  async createFileRecordAndProcess(
    userId: string,
    fileData: {
      fileName: string;
      mimeType: string;
    ...
    try {
      ...
      console.log(
        `✅ [CreateFileRecord] Downloaded ${fileBuffer.length} bytes`
      );
    } catch (error) {
      console.error(
